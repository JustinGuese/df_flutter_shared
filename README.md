...
| **[df_ui_widgets](df_ui_widgets/)** | Reusable UI: `QuickActionChip` (optional gradient/colors), `SummaryBulletList` (bullets, â€œ+X moreâ€, optional title/icon), `KeywordChipList` (tags + AI pulse indicator), `LoadingAppBarAction`. Theme-based defaults; pass app colors/gradients for branding. |
